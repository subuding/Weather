...
public class AstroView extends View {
	...
	private HeWeather6.HeWeather6Bean.DailyForecastBean todayForecast;
	private HeWeather6.HeWeather6Bean.NowBean now;
//	private PorterDuffXfermode clearfXfermode = new PorterDuffXfermode(Mode.CLEAR);
	private float sunArcHeight, sunArcRadius;

	private Rect visibleRect = new Rect();

	public void setData(HeWeather6 weather) {
		try {
			if (ApiManager.acceptWeather(weather)) {
				this.now = weather.getHeWeather6().get(0).getNow();
				final HeWeather6.HeWeather6Bean.DailyForecastBean forecast = weather.getHeWeather6().get(0).getDaily_forecast().get(0);
				if (forecast != null) {
...
//		setLayerType(View.LAYER_TYPE_SOFTWARE, null);
	...
	protected void onDraw(Canvas canvas) {
		...
		try {
			...
			canvas.drawText(now.getWind_spd() + "km/h " + now.getWind_dir(), fanHeight + textSize, 0, paint);
			...
			float speed = 0f;
			try {
				speed = Float.valueOf(now.getWind_spd());
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			paint.setTextAlign(Align.CENTER);
			canvas.drawText("日出 " + todayForecast.getSr(), textLeft, textSize * 10.5f + paintTextOffset, paint);
			canvas.drawText(todayForecast.getSs() + " 日落", width - textLeft, textSize * 10.5f + paintTextOffset, paint);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		try {
			// draw the sun
			String[] sr = todayForecast.getSr().split(":");// 日出
			int srTime = Integer.valueOf(sr[0]) * 60 * 60 + Integer.valueOf(sr[1]) * 60 + 0;// 精确到秒
			String[] ss = todayForecast.getSs().split(":");// 日落
