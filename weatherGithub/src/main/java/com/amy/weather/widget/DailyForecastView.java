...
public class DailyForecastView extends View {
	...
	private List<HeWeather6.HeWeather6Bean.DailyForecastBean> forecastList;
...
//			tmpMinPath.quadTo(x[i], yMin[i], midX, midYMin);
	...
	public void setData(HeWeather6 weather) {
		...
		if(this.forecastList == weather.getHeWeather6().get(0).getDaily_forecast()){
			percent = 0f;
			invalidate();
			return ;
		}
		this.forecastList = weather.getHeWeather6().get(0).getDaily_forecast();
		...
		try {
			...
			for (int i = 0; i < forecastList.size(); i++) {
				HeWeather6.HeWeather6Bean.DailyForecastBean forecast = forecastList.get(i);
