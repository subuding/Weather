...
public class HourlyForecastView extends View {
	...
	private List<HeWeather6.HeWeather6Bean.HourlyBean> forecastList;
...
//		}
	...
	public void setData(HeWeather6 weather) {
		...
		if (this.forecastList == weather.getHeWeather6().get(0).getHourly()) {
//			percent = 0f;
			invalidate();
			return;
		}
		try {
			final List<HeWeather6.HeWeather6Bean.HourlyBean> w_hourlyForecast = weather.getHeWeather6().get(0).getHourly();
			if (w_hourlyForecast.size() == 0) {// 有可能为空
				return;
			}
			if (!ApiManager.isToday(w_hourlyForecast.get(0).getTime())) {// 不是今天的数据
				return;
			}
			...
			for (int i = 0; i < forecastList.size(); i++) {
				HeWeather6.HeWeather6Bean.HourlyBean forecast = forecastList.get(i);
